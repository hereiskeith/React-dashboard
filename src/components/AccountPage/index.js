...
const AccountPage = props => {
  ...
  const handleValueChanged = (event, inputState, name) => {
    switch (inputState) {
      ...
      case 'forgetPassword':
        return setForgetPasswordFormValue({...forgetPassword, [name]: event.target.value});
      default:
        return;
    }
  };
  ...
  const handleSignUpBtn = () => {
    ...
    if(inputState === 'login') {
      ...
      const errorExist = Object.values(error.toJS()).some(value => value === true);
      if(errorExist) {
  ...
  return (
    <div className='account-wrapper'>
      ...
      <div className={'account' + (inputState === 'signUp'?' account-sign-up':'')}>
        ...
        <div className='typography-wrapper'>
          <CSSTransition in={inputState === 'login'} timeout={500} classNames='flip' unmountOnExit>
        ...
        <div className='mobile-wrapper'>
          ...
          <div className='input-wrapper'>
            <Input
              state={inputState === 'signUp'} type='text' name='first name' placeholder='First Name'
              value={valueInDiffForm(inputState, 'firstName')}
              onValueChange={(event) => handleValueChanged(event, inputState,'firstName')}
              submitting={submitState === 'submitting'}/>

            <Input
              state={inputState === 'signUp'} type='text' name='last name' placeholder='Last Name'
              value={valueInDiffForm(inputState, 'lastName')}
              onValueChange={(event) => handleValueChanged(event, inputState,'lastName')}
              submitting={submitState === 'submitting'}/>

            <Input
              state={inputState !== 'forgetPassword'} type='text' name='username' fullLength
              placeholder='Username' value={valueInDiffForm(inputState, 'userName')}
              onValueChange={(event) => handleValueChanged(event, inputState,'userName')}
              submitting={submitState === 'submitting'}
              customErrorNotes='Your username should be within 4-10 word characters'/>

            <Input
              state={inputState !== 'login'} type='email' name='email' fullLength
              placeholder='Email' value={valueInDiffForm(inputState, 'email')}
              onValueChange={(event) => handleValueChanged(event, inputState,'email')}
              submitting={submitState === 'submitting'}/>

            <Input
              state={inputState !== 'forgetPassword'} type='password' name='password' fullLength
              placeholder='Password' value={valueInDiffForm(inputState, 'password')}
              onValueChange={(event) => handleValueChanged(event, inputState,'password')}
              submitting={submitState === 'submitting'}
              customErrorNotes='Your password should be more than 5 word characters'/>

            <Input
              state={inputState === 'signUp'} type='password' name='confirmed password' fullLength
              placeholder='Confirm password' value={valueInDiffForm(inputState, 'confirmPassword')}
              onValueChange={(event) => handleValueChanged(event, inputState,'confirmPassword')}
              submitting={submitState === 'submitting'}
              isValid={!!(signUp.password && signUp.password === signUp.confirmPassword)}
              customErrorNotes={'The password doesn\'t match above.'}/>
          </div>

          <div className='no-match-error-text'>
            {(login.userName !== userName || login.password !== password) && (loginState === 'loginFailed')?
          ...
          <div className='checkbox-wrapper'>
            <Checkbox
              ...
              onChange={() => setLoginFormValue({...login, 'rememberMe': !(login.rememberMe)})}/>
          ...
          <div className='checkbox-wrapper'>
            <Checkbox
              ...
              submitting={submitState === 'submitting'}
              onChange={() => {
                setSignUpFormValue({...signUp, 'agreeTerms': !signUp.agreeTerms})
              }}
              animationState={animationState}/>
          </div>

          <div className={'button-wrapper' + (inputState !== 'login'?' button-wrapper-sign-up':'')}>
            <Button
...
const mapDispatch = dispatch => ({
  signUpNewUser(signUpData, setInputState, setSubmitState) {
    dispatch(actionCreators.signUpNewUser(signUpData, setInputState, setSubmitState))
  }
});
