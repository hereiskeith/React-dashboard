...
const Dashboard = (props) => {
  ...
  let mostRecentTurnoverBuy = `$${(
    turnoverBuy[turnoverBuy.length - 1][1] / 1000
  ).toFixed(2)}K`;
  let mostRecentPercentageBuy = `${(
    (turnoverBuy[turnoverBuy.length - 1][1] /
      turnoverBuy[turnoverBuy.length - 2][1] -
      1) *
    100
  ).toFixed(2)}%`;

  let mostRecentTurnoverSell = `$${(
    turnoverSell[turnoverSell.length - 1][1] / 1000
  ).toFixed(2)}K`;
  let mostRecentPercentageSell = `${(
    (turnoverSell[turnoverSell.length - 1][1] /
      turnoverSell[turnoverSell.length - 2][1] -
      1) *
    100
  ).toFixed(2)}%`;

  let mostRecentTurnoverNet = `$${(
    turnoverNet[turnoverNet.length - 1][1] / 1000
  ).toFixed(2)}K`;
  let mostRecentPercentageNet = `${(
    ((turnoverNet[turnoverNet.length - 1][1] -
      turnoverNet[turnoverNet.length - 2][1]) /
      Math.abs(turnoverNet[turnoverNet.length - 2][1])) *
    100
  ).toFixed(2)}%`;
  ...
  return (
    <div className="dashboard-wrapper">
      ...
      <HighchartsComponent className="bitcoin-price" options={bitcoinPrice} />
