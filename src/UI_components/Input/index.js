...
const Input = props => {
  const { setErrorBool, state, name, value, isValid, submitting, customErrorNotes } = props; 
  ...
  return (
    <CSSTransition in={state} timeout={500} classNames='flip-input' unmountOnExit>
      <div className={'c-input-wrapper ' + (fullLength ? 'c-input-wrapper-full-length ' : '')} >
      <input
        required={true}
        type={type}
        name={name}
